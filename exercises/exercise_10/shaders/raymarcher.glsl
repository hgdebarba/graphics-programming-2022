...
vec3 calculateNormal(vec3 p, float h)
{
    ...
    for(int i = ZERO; i < 4; i++)
    {
        ...
        normal += e * GetDistance(p + e * h, output);
    }
...
{
    Output output; // intentionally unused, to help compiler optimize
    float distance = 0.0f;
...
void main()
{
    ...
    Output output;
    InitOutput(output);
    GetDistance(point, output);

    // With the output value, get the final color
    FragColor = GetOutputColor(point, distance, output);

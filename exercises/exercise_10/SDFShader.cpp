...
SDFShader::SDFShader(const char* vertexPath, const char* fragmentPath)
    : m_Program(0)
...
void SDFShader::Use() const
{
    glUseProgram(m_Program);
}

void SDFShader::Reload() const
{
    ...
    if (vertexRead && fragmentRead)
    {
        ...
        if (!RayMarcher::HasSourceChanged() && m_VertexHash == prevVertexHash && m_FragmentHash == prevFragmentHash)
            return;
        ...
        const char* fragmentSources[] = { RayMarcher::GetSDFLibrarySource(), fragmentShaderCode.c_str(), RayMarcher::GetRayMarcherSource() };
...
bool SDFShader::ReadShaderFile(const char* path, std::string& shaderCode)
{
    bool succeded = false;
...
bool SDFShader::CheckShaderErrors(GLuint shader)
{
    GLint success;
...
GLint SDFShader::GetUniformLocation(const char* name) const
{
    int location = -1;
    if (m_Program)
    {
...
unsigned int SDFShader::GetUniformIndex(GLint location) const
{
    return m_LocationUniforms.find(location)->second;
}

unsigned int SDFShader::GetUniformIndex(const char* name) const
{
...
void SDFShader::SetUniform(const char* name, const void* data) const
{
...
unsigned int SDFShader::GetUniformCount() const
{
    return (unsigned int)m_Uniforms.size();
}

void SDFShader::SetUniform(unsigned int index, const void* data) const
{
...
void SDFShader::GetUniform(unsigned int index, void* data) const
{
    if (m_Program && index != ~0u)
    {
        const Uniform& uniform = m_Uniforms[index];
...
unsigned int SDFShader::GetUniformSize(unsigned int index) const
{
    const Uniform& uniform = m_Uniforms[index];
...
void SDFShader::LoadActiveUniforms()
{
    GLint maxNameLength;
...
void SDFShader::GetTypeInfo(GLenum type, unsigned int& size) const
{
    GetFunction getFn;
    SetFunction setFn;
...
void SDFShader::GetTypeInfo(GLenum type, unsigned int& size, GetFunction& getFn, SetFunction& setFn) const
{
    GLenum innerType;
    switch (type)
    {
